...
function printHello()
{
    // display a message in the 'printHello'
...
{
    // a and b are parameters that can be used in the function statement
...
for (var i = 0; i < fruits.length; i++)
{
    ...
    if (i != fruits.length - 1)
        ...
        // if we get o the last index in the array
        output += fruits[i] + "]"; 
}
...
var count = Math.floor(Math.random() * 11);    // Math.random() generates a decimal between 0 and 1
...
// function that multiples a value times 3
/*
...
var count = 5 + Math.floor(Math.random() * 11); // Math.random() generates a decimal between 0
                                            // and 1
                                // multiplying times 11 generates a decimal between 5 and 15 or 16
// use a for loop to add count numbers to the array .push()
for (var i = 0; i <= count; i++)
{
    numbers.push(Math.floor(Math.random() * 100)); // generate a value between 0 and 100 and add to the array
...
var sorted2 = numbers.sort(function sortFunction(a, b) {
    // where 'a' and 'b' are values in adjacent sequential indexes
...
output = "[";
// populate using the .forEach() method
sorted2.forEach((number, index) => {
    if (index != sorted2.length - 1)
        output += number + ", ";
    else
        output += number + "]";
});
...
// can also use .reverse() function to reverse the order of the elements
